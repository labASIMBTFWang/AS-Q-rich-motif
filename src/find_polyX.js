...
function any_polyAA_impure(ss, min_num_AA, AA, max_num_not_AA = 2) {
	...
	if (mm_arr.length) {
		...
		}).filter(v => v.num_AA >= min_num_AA).map(({ part: a, num_AA, frag }) => {
 ...
 * @param {number} max_num_not_AA
 */
function findPolyXX(input_seq, min_num_AA, amino_acid, max_num_not_AA = 2) {
...
module.exports.findPolyXX = findPolyXX;
